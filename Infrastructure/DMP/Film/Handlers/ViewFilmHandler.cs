...
public class ViewFilmHandler : IViewFilmHandler
{
    private readonly IFilmManagementService _filmManagementService;
    private readonly ILoggerService _loggerService;
    private readonly IMapper _mapper;

    public ViewFilmHandler(IFilmManagementService filmManagementService, ILoggerService loggerService, IMapper mapper)
    {
    ...
    public async Task<Result<ViewFilmResponse>> Handle(ViewFilmQuery query, CancellationToken cancellationToken)
    {
        try
        {
            var result = await _filmManagementService.ViewFilmAsync(query.Id, cancellationToken);
            if (result.Succeeded)
